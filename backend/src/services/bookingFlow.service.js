// @ts-check
/**
 * What happens to a booking, from the moment somebody commits to it.
 *
 * Everything in here used to live in booking.controller.js, which had grown to
 * 1,113 lines and held four jobs at once: parsing HTTP, pricing, moving money,
 * and shaping responses. The cost of that is not tidiness — it is that the
 * hardest logic in the product, the part that debits wallets and issues
 * refunds, could only be reached through an Express request. You cannot call
 * it from a script, a queue, or a test without building a fake req.
 *
 * The split is by job, not by size:
 *
 *   booking.service.js       what a slot costs and whether it is free —
 *                            pure domain logic, no writes
 *   this file                what happens when somebody books, cancels,
 *                            confirms or settles — orchestration and money
 *   booking.controller.js    reads the request, calls one of these, responds
 *
 * Nothing here knows about `req` or `res`. Each function takes the acting user
 * and plain values, returns a plain object, and throws ApiError — which the
 * controller's asyncHandler turns into a response exactly as before.
 */
...
import { quoteBooking, refundFor, claimPromoUse } from './booking.service.js';
...
import { toLabel, toDate } from '../utils/time.js';
...
export async function loadVenueAndCourt(venueId, courtId) {
...
export async function createBooking(actor, input) {
  const { venueId, courtId, date, starts, promoCode, players = 1, notes = '', paymentMethod } = input;
   ...
   */
  if (String(venue.owner) === String(actor._id)) {
  ...
  const q = await quoteBooking({
    ...
    tierKey: actor.loyaltyTier, userId: actor._id,
  });
   ...
   */
  const promoClaim = await claimPromoUse(q.promoResolved, promoCode, actor._id);
  ...
  if (isInstant && settlesFromWallet && q.total > 0) {
    try {
      await wallet.debit(actor, q.total, {
        booking: docs[0],
  ...
  if (isInstant && docs[0]?.payment?.status === 'paid') {
    loyaltyResult = await loyalty.award(actor, q.pointsToEarn, {
  ...
  await notify.notify(actor._id, confirmedNow ? 'booking_confirmed' : 'booking_requested', {
  ...
  if (!isInstant) {
    await notify.notify(venue.owner, 'teamup_request', {
      ...
      body: `${actor.name} requested ${court.name} at ${venue.name} on ${date}.`,
      link: '/owner/requests',
      icon: '📞',
    });
  }
  ...
  return {
    booking: group,
    walletBalance: actor.walletBalance,
    loyalty: loyaltyResult,
    loyaltyPoints: actor.loyaltyPoints,
    loyaltyTier: actor.loyaltyTier,
 ...
 */
export async function cancelBooking(actor, groupRef, { reason = '' } = {}) {
  const rows = await Booking.find({ groupRef });
  if (!rows.length) throw ApiError.notFound('Booking not found');

  const isOwnerOfBooking = String(rows[0].user) === String(actor._id);
  ...
  const flip = await Booking.updateMany(
    { groupRef, status: { $in: [BOOKING_STATUS.PENDING, BOOKING_STATUS.CONFIRMED] } },
    {
      $set: {
        status: BOOKING_STATUS.CANCELLED,
        slotLocked: false,                      // frees the slot for everyone else
        'cancellation.cancelledAt': now,
        'cancellation.cancelledBy': actor._id,
        'cancellation.reason': cleanText(reason, 300),
      },
    }
  );
  ...
  const bookingUser = isOwnerOfBooking ? actor : await User.findById(rows[0].user);
...
export async function decideBooking(actor, groupRef, decision) {
  const rows = await Booking.find({ groupRef });
  if (!rows.length) throw ApiError.notFound('Booking not found');

  const venue = await Venue.findById(rows[0].venue);
  if (!venue) throw ApiError.notFound('Venue not found');
  if (String(venue.owner) !== String(actor._id) && actor.role !== 'admin') {
 ...
 */
export async function settleCash(actor, groupRef) {
  const rows = await Booking.find({ groupRef });
  if (!rows.length) throw ApiError.notFound('Booking not found');

  const venue = await Venue.findById(rows[0].venue).select('owner name');
  if (!venue) throw ApiError.notFound('Venue not found');
  if (String(venue.owner) !== String(actor._id) && actor.role !== ROLES.ADMIN) {
